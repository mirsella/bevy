...
pub fn init_sprite_pipeline(
    ...
    commands.insert_resource(SpritePipeline {
        ...
        shader: load_embedded_asset!(&*asset_server, "sprite.wgsl"),
    });
}
...
pub fn init_srgb_sprite_pipeline(mut commands: Commands, sprite_pipeline: Res<SpritePipeline>) {
    commands.insert_resource(SrgbSpritePipeline {
        sprite_pipeline: sprite_pipeline.clone(),
    });
}
...
pub fn extract_srgb_sprite_camera_phases(
    ...
    for (main_entity, camera) in &cameras {
        ...
        let retained_view_entity =
            RetainedViewEntity::new(main_entity.into(), Option::<MainEntity>::None, 0);
            ...
            match extracted_sprite.kind {
                ...
                ExtractedSpriteKind::Slices { ref indices } => {
                    for i in indices.clone() {
                        ...
                        if extracted_sprite.flip_x {
                            // Text slices do not currently use atlas flipping.
                        }
                        if extracted_sprite.flip_y {
                            // Text slices do not currently use atlas flipping.
                        }
...
impl ViewNode for SrgbSpritePassNode {
    ...
    fn run<'w>(
        ...
        (srgb_texture, _view_uniform_offset, view): QueryItem<'w, '_, Self::ViewQuery>,
        world: &'w World,
    ) -> Result<(), NodeRunError> {
...
impl<P: PhaseItem, const I: usize> RenderCommand<P> for SetSpriteViewBindGroup<I> {
    ...
    fn render<'w>(
        _item: &P,
        (view_uniform, sprite_view_bind_group): QueryItem<'w, '_, Self::ViewQuery>,
        _entity: Option<()>,
