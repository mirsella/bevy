...
#ifdef MANUAL_SRGB
    return vec4(pow(clamp(color.rgb, vec3(0.0), vec3(1.0)), vec3(1.0 / 2.2)), saturate(color.a * t * nearest_border));
#else
...
fn draw_uinode_background(
    ...
    // Encode to sRGB so blending in Rgba8Unorm happens in sRGB/gamma space
#ifdef MANUAL_SRGB
    return vec4(pow(clamp(color.rgb, vec3(0.0), vec3(1.0)), vec3(1.0 / 2.2)), saturate(color.a * t));
#else
